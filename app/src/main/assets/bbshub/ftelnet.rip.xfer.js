//
// THE Z-MODEM MOD [ 2019 ]
// 

var ZModemFileName = "TEXT.000x001.TXT";
...
var Crt = (function () {
    function Crt(container, useModernScrollback) {
        ...
        this._AllowDynamicFontResize = false;
        this._Atari = false;
        ...
        this._TempCanvasContext.font = '10pt monospace';
    ...
    CrtFont.prototype.Load = function (font, maxWidth, maxHeight) {
        ...
        if (font.indexOf('_') >= 0) {
            if (CrtFonts.HasFont(font)) {
               ...
               console.log('Sorry we do not have this font');
            }
        } else {
        ...
        if (typeof BestFit === 'undefined') {
            ...
            alert('fTelnet Error: Font CP=' + font + ' does not exist');
            return false;
        } else {
    ...
    WebSocketConnection.prototype.connect = function (hostname, port, urlPath, forceWss, proxyHostname, proxyPort, proxyPortSecure) {
        ...
        if (UseCordovaSocket) {
            ...
            if (proxyHostname === '') {
                ...
                //this._WebSocket = new WebSocket(WsOrWss + '://' + proxyHostname + ':' + (WsOrWss === 'wss' ? proxyPortSecure : proxyPort) + '/ssh/' + 'new' + '/' + 'new' + '/' + hostname + '/' + port, Protocols);
                this._WebSocket = new WebSocket(WsOrWss + '://' + proxyHostname + ':' + (WsOrWss === 'wss' ? proxyPortSecure : proxyPort) + '/' + hostname + '/' + port);
                //this._WebSocket = new WebSocket(WsOrWss + '://' + proxyHostname + ':' + (WsOrWss === 'wss' ? proxyPortSecure : proxyPort) + '/ssh/' + 'new' + '/' + 'new' + '/' + hostname + '/' + port);
            }
...
var CrtProgressBar = (function (_super) {
    ...
    function CrtProgressBar(crt, parent, left, top, width, style) {
        ...
        _this._Style = style;
        _this.BackColour = Crt.BLACK;
        _this._BarForeColour = Crt.DARKGRAY;
...
var CRC = (function () {
    ...
    CRC.UpdateCrcBin = function (curByte, curCrc) {
      return (this.CRC_TABLE[(curCrc >> 8) & 0x00FF] ^ (curCrc << 8) ^ curByte) ;
    };
...
// 20190716 QUEST FOR MISSING BYTE 
// > ls -l VGA32PSF.ZIP; md5sum VGA32PSF.ZIP 
// c544506ab67cd9be09486904a991d096  VGA32PSF.ZIP
// 
// V> ls -l VGA32PSF.ZIP; md5sum VGA32PSF.ZIP
// f7a7b19f83e8fbdcdd6a10845fb9ee7d  VGA32PSF.ZIP
//
// 20190716 QUEST FOR ADDITINAL BD // IN THE CREW.ZIP FILE 
//THERE IS ADDITIONAL BD AT 000007e0 0x2b (43)
...
var ZModemReceive = (function () {
  ...
  function ZModemReceive(crt, connection) {
    ...
    this.ZDLEE = String.fromCharCode(88);
    this.ZHEX = 'B';
    this.ZBIN = 'A';
  }


  ZModemReceive.prototype.calcCRCBytesBin = function(data) {
    //var crc = 0xffffffff;
    var crc = 0xffff;
    var d;
    data.position = 0;
    for (i = 0; i < data.length; i++) {
      ...
      crc = CRC.UpdateCrcBin(d,crc);
    }
    return (crc);
  }

  ZModemReceive.prototype.calcCRCBytes = function(data) {
    //var crc = 0xffffffff;
    var crc = 0xffff;
    var d;
    data.position = 0;
    for (i = 0; i < data.length; i++) {
      d = data.readUnsignedByte();
      crc = CRC.UpdateCrc(d,crc);
    }
    return (crc);
  }

  ZModemReceive.prototype.calcCRC = function(data) {
    var crc = 0x0000;
    var d;
    data.position = 0;
    for (i = 0; i < data.length; i++) {
  ...
  ZModemReceive.prototype.hexToBin = function(val) {
    var tmp = parseInt(val,16).toString(2).substr(-8);
    var ret = tmp;
    return(ret);
  }

  ZModemReceive.prototype.Hex4ToBin = function(val) {
    str = String.fromCharCode(val.toString(val));
    var tmp = val.toString(16);
    var ret = ("000"+tmp).slice(-4).toLowerCase();
    return(ret);
  }

  ZModemReceive.prototype.joinHex4 = function(val) {
    var tmp = val.toString(16);
    var ret = tmp.toLowerCase();
    return(ret);
  }
  ...
  ZModemReceive.prototype.packetZCRC=function(value) {
    ZPAD='*'; // PAD CHAR
    ZDLE=String.fromCharCode(24); // ZDLE ESCAPE SEQ
    ZHEX='B'; // HEX HEADER
    CR=0x0D; // CARRIAGE RETURN
    LF=0x0A; // LINE FEED
    PTP=0x0D; // PACKET TYPE [ ZCRC ]
    HDR=this.IToChar(ZPAD)+this.IToChar(ZPAD)+this.IToChar(ZDLE)+this.IToChar(ZHEX)+this.IToHex(PTP);
    DAT=this.IToHex8(value);
    END=this.IToHex(CR)+this.IToHex(LF);
    PKT=HDR+DAT+this.IToHex4(this.calcCRC(this.hexToBytes(HDR+DAT)))+END;
    return (PKT);
  }

  ZModemReceive.prototype.packetZACK=function(position) {
    ZPAD='*'; // PAD CHAR
    ZDLE=String.fromCharCode(24); // ZDLE ESCAPE SEQ
    ZHEX='B'; // HEX HEADER
    CR=0x0D; // CARRIAGE RETURN
    LF=0x0A; // LINE FEED
    PTP=0x03; // PACKET TYPE [ ZACK ]
    HDR=this.IToChar(ZPAD)+this.IToChar(ZPAD)+this.IToChar(ZDLE)+this.IToChar(ZHEX)+this.IToHex(PTP);
    DAT=this.IToHex8(position);
    END=this.IToHex(CR)+this.IToHex(LF);
    PKT=HDR+DAT+this.IToHex4(this.calcCRC(this.hexToBytes(HDR+DAT)))+END;
    return (PKT);
  }

  ZModemReceive.prototype.packetZFIN=function(position) {
    ZPAD='*'; // PAD CHAR
    ZDLE=String.fromCharCode(24); // ZDLE ESCAPE SEQ
    ZHEX='B'; // HEX HEADER
    CR=0x0D; // CARRIAGE RETURN
  ...
  ZModemReceive.prototype.packetZRPOS=function(position) {
    ZPAD='*'; // PAD CHAR
    ZDLE=String.fromCharCode(24); // ZDLE ESCAPE SEQ
    ZHEX='B'; // HEX HEADER
    CR=0x0D; // CARRIAGE RETURN
  ...
  ZModemReceive.prototype.packetZRINIT=function(par_flag) {
    ZPAD='*'; // PAD CHAR
    ZDLE=String.fromCharCode(24); // ZDLE ESCAPE SEQ
    ZHEX='B'; // HEX HEADER
    CR=0x0D; // CARRIAGE RETURN
    ...
    PKT_HDR=this.IToChar(ZPAD)+this.IToChar(ZPAD)+this.IToChar(ZDLE)+this.IToChar(ZHEX)+this.IToHex(PTP);
    PKT_DAT=this.IToHex(ZP0)+this.IToHex(ZP1)+this.IToHex(ZF1)+this.IToHex(ZF0);
    PKT_END=this.IToHex(CR)+this.IToHex(LF);
    PKT=PKT_HDR+PKT_DAT+this.IToHex4(this.calcCRC(this.hexToBytes(PKT_HDR+PKT_DAT)))+PKT_END;
    return (PKT);
  }

  ZModemReceive.prototype.receivePacket = function (BlockSize) {
  ...
  ZModemReceive.prototype.receiveUnsignedBytes = function (BlockSize) {

    var byteHdr= [];
    for (w=0;w<BlockSize;w++) {
      c = this._NextByte = this._Connection.readUnsignedByte();
      byteHdr.push(c);
      //console.log(c);
    }
    return(byteHdr);
  ...
  ZModemReceive.prototype.Cancel = function (reason) {
    ...
    RECV_STEP = 0; // PREPARE FOR NEW DOWNLOAD
  ...
  ZModemReceive.prototype.CleanUp = function (message) {
    ...
    setTimeout(function () { _this.Dispatch(); }, 4000);
    RECV_STEP = 0; // HERE WE START AGAIN
  };
  ...
  ZModemReceive.prototype.Download = function () { // HERE
    var _this = this;
    this._Timer = setInterval(function () { _this.OnTimer(); }, 500);
    //this._Timer = setInterval(function () { _this.OnTimer(); }, 350);
    //this._Timer = setInterval(function () { _this.OnTimer(); }, 50);
    this._Crt.HideCursor();
    // ADD NUMBER OF PACKETS TBD
    this.pnlMain = new CrtPanel(this._Crt, undefined, 10, 5, 60, 14, BorderStyle.Single, Crt.WHITE, Crt.BLACK, 'ZModem Receive Status (Hit CTRL+X to abort)', ContentAlignment.TopLeft);
  ...
  ZModemReceive.prototype.unpack16BE = function(array) {
  ...
  ZModemReceive.prototype.invertBit = function (v, digits) {
  ...
  ZModemReceive.prototype.printPacketDebug = function (pkt) {
    ...
    B = pkt.readUnsignedByte();
    while ((B != this.ZDLE) && (pkt.bytesAvailable > 0)) {
      B = pkt.readUnsignedByte();
      pktBytes.push(B);
      ...
      pktHex.push(H);
      if (B!=10) {
        pktDebug.push(B + ' ' + C + ' 0x' + H);
      } else {
        pktDebug.push(B + ' LF ' +  '0x' + H);
      }
    }

    console.log("RECEIVED ZDATA DEBUG (Byte Char Hex): \n" + pktDebug);

    var dataBegin = 0;
    var dataEnd = 0;
    var BytesPosition = 0;
    var dataPacket = new ByteArray();
    var dataCRC = new ByteArray();
    var dpi=0;
    //var ZDLE=24;
    //var ZDLEE=88;
    console.log();
    for (i = 0; i < pktString.length - 15; i++) {

      var byte = pktBytes[i];

      if (dataBegin===1) {

        if (byte === 24) { // this.hexToBytes(this.ZDLE))
          console.log(" ESC " + byte + " H: " + this.IToHex(byte));
          i++;
        } else {
          dataBytes.push(byte);
        }
        dataString.push(byte);
        dpi++;
      }
      if (i===8) {
        dataBegin=1;
    ...
    dataPacket.position = dpi;

    console.log('RECEIVED DATA (BYTES):'  + dataBytes + " (STRING): " + dataString);

    return(dataBytes);

    /*
 
      RECEIVED ZDATA FRAME DEBUG: 
        0000 [1] 24 � 0x18 (ZDLE) // ZDATA HEADER
        0001 [2] 65 A 0x41 (BINARY HEADER WITH CRC16)
        0002 [3] 10 LF  0x0a (Type of Packet - ZFILE)
        0003 [4] 0 � 0x00 - FLAG3
        0004 [5] 0 � 0x00 - FLAG2
        0005 [6] 0 � 0x00 - FLAG1
        0006 [7] 0 � 0x00 - FLAG0
        0007 [8] 70 F 0x46 - RESERVED 1
        0008 [9] 174 ® 0xae  - RESERVERD 2, 
      FILE DATA BLOCK: 
        0009 [0] 48 0 0x30
        0010 [1] 49 1 0x31
        0011 [2] 50 2 0x32
        ...
        0018 [9] 10 LF  0x0a
      : END OF DATA
        0019 [1] 24 � 0x18 (ZDLE) // ZEOF HEADER 
        0020 [2] 104 h 0x68 (WHO THE HELL KNOWS WHAT'S THIS BELOW)
        0021 [3] 167 § 0xa7
        0022 [4] 93 ] 0x5d
        0023 [5] 42 * 0x2a
        0024 [6] 24 � 0x18
  ...
  ZModemReceive.prototype.IToBin = function(value) {
    return("000000000" + value.toString(2).substr(-8));
  }
 
  ZModemReceive.prototype.printBytesAsBin = function(array,start,end) {
    var binTxt = '' ;
    var binData = [];
    array.position=start;
    for (i=0;i<end-start;i++) {
      ub=array.readUnsignedByte();
      binTxt+=this.IToBin(ub) + '';
      binData.push(this.IToBin(ub));
    }
    console.log('printBytesAsBin:' + binTxt + 'SZ: ' + binData.length);
    return(binData);
  }

  ZModemReceive.prototype.printBytesAsHex = function(array,start,end) {
    // GET CRC (Find End of the ZDATA frame)
    var hexTxt = '' ;
    var hexData = [];
    array.position=start;
    for (i=0;i<end-start;i++) {
      ub=array.readUnsignedByte();
      hexTxt+=this.IToHex(ub) + ' ';
      hexData.push(this.IToHex(ub));
    }
//    console.log('printBytesAsHex:' + hexTxt + 'SZ: ' + hexData.length);
    return([hexData,hexTxt]);
  }

  ZModemReceive.prototype.bytesSubFrame = function(array,start,end) {
    // GET CRC (Find End of the ZDATA frame)
    var subFrame = new ByteArray();
    array.position=start;
    subFrame.position = 0;
    for (i=0;i<end-start;i++) {
      ub=array.readUnsignedByte();
      subFrame.writeByte(ub);
    }
    subFrame.position=end-start;
    subFrame.length=end-start;
    //console.log('SUBFRAME ' + this.printBytesAsHex(subFrame)+ 'SZ: ' + subFrame.length);

    return(subFrame);

  }

  // DISESCAPE - JUST DROPS ZDLE
  ZModemReceive.prototype.bytesDisescape = function(array) {
    ...
    try {
      ...
      unesc.position = pos;
       ...
       console.log('Disescape EXCEPTION: ' + pos + ' UNESC ' + unesc + ' AVAIL: ' + array.bytesAvailable);
    }
    return(unesc);
  }

  // UNESCAPING ZDLE
  ZModemReceive.prototype.bytesUnescape = function(array) {
  ...
  ZModemReceive.prototype.bytesUnframe = function(array,posInFile) {

    var unfra = new ByteArray();
    var debugUnfra = new ByteArray();
    arrayHex=this.printBytesAsHex(array,0,array.length);
    var byt;
    var pos=0;
    var UnfraPos=0; 
    var unfraBytes=0;
    var unfraStart=0;
    var debugHex="";
    var crcArray = new ByteArray();
    try {
      UnfraPos=this.findHexSeq(arrayHex[0],'18','69'); // ZRRCG
      unfraBytes=6;
      debugUnfra.writeBytes(array,UnfraPos,unfraBytes);
      debugHex=this.printBytesAsHex(debugUnfra,0,debugUnfra.length);

      unfraBytes=4;
      if (debugHex[0][0]!=18) { return(array); }
      if (debugHex[0][2]==18 || debugHex[0][3] == 18) { unfraBytes=5; }
      if (debugHex[0][2]==18 && debugHex[0][4] == 18) { unfraBytes=6; }
      //console.log("Unfra DEBUG HEX: " + debugHex[1]);

      crcArray.position=0;
      crcArray.writeBytes(array,UnfraPos+2,unfraBytes-2);
      //console.log("CRC " + this.printBytesAsHex(crcArray,0,crcArray.length)[1]);

      unfra=this.removeBytes(array,UnfraPos,unfraBytes);
      unfra.position = unfra.length;

      //console.log("UnfraPos ["+unfraBytes+"] H " + this.IToHex4(UnfraPos) + " D " + UnfraPos + " AL " + array.length + " PIF " + this.IToHex4(posInFile) + " UL " + unfra.length + " CRC: " + this.IToHex4(this.bytesUnescape(crcArray)));
    } catch(esc) {
       console.log('EXCEPTION: ' + pos + ' UNFRA ' + unfra + ' AVAIL: ' + array.bytesAvailable);
       console.log(esc);
    }
    //console.log(unfra);
    return(unfra);
  ...
  ZModemReceive.prototype.findHexSeq = function(array,hex01,hex02) {
    var HEX = "";
    var pos=0;
    for (i=0;i<array.length-1;i++) {
      dat01=array[i];
      dat02=array[i+1];
      if (dat01==hex01 && dat02==hex02) {
        HEX=hex01+hex02;
  ...
  // zDataPos=this.findHexSeq(zDataHex,'46','ae');
  ZModemReceive.prototype.findHex = function(array,hex01) {
    var HEX = "";
    var pos=0;
    for (i=0;i<array.length-1;i++) {
      dat01=array[i];
  ...
  ZModemReceive.prototype.removeZDLEBytes = function(array) {
   
    var tmpData = new ByteArray();
    array.position = 0;

    for (i=0;i<array.length;i++) {
      val = tmpData.readUnsignedByte();
      if (val!=0x18) {
        arra.writeByte(val);
      }
    }
  ...
  ZModemReceive.prototype.removeBytes = function(array,pos,count) {
   
    //console.log("Unfra BF fix1k " + array.length + " POS: " + this.IToHex4(pos) + " CNT: " + count);
    var tmpData = new ByteArray();
    array.position = 0;
    tmpData.position=0;
    tmpData.writeBytes(array,0,pos);
    tmpData.position=pos;
    tmpData.writeBytes(array,pos+count,array.length-pos-count); // PROBABLY - COUNT
    array.position=0;
    tmpData.position=0;
    array.writeBytes(tmpData,0,tmpData.length);
    array.position=tmpData.length;
    array.length=tmpData.length;
    //console.log("Unfra O.K. " + array.length + " " + tmpData.length);

    return (array);
  }


  ZModemReceive.prototype.headerCRC = function (array,start,end) {
    var debug = new ByteArray(); 
    txtDebug = "";
    array.position = start;
    debug.position = 0;
    for (i=start;i<end;i++) {
      ub=array.readUnsignedByte();
      debug.writeByte(ub);
      txtDebug += this.IToHex(ub) + ' ';
    }
    ourCRC=this.IToHex4(this.crlcr(debug));
    console.log('CRC: ' + ourCRC  + ' DATA: ' + txtDebug)
  }
  ...
  var RECV_ZFILE = 0;
  var RECV_ZDATA = 0;
  var senderFileSize = 0;
  var missingBytes = 0;
  var RECV_STEP = 0;
  var receivedDataSize = 0;
  var receivedDataCheck = 0;
  var recvPacketCount = 0;
  var retryPacketCount = 0;
  var zackSent = 0;
  var rawDataRecv = 0;
  var realDataRecv = 0;
  var fix10k = 0;
  ...
  ZModemReceive.prototype.OnTimer = function () {
    ...
    if (RECV_STEP === -1) {
      ...
      // SEND ZM
      // SELECT DISCONNECT YES
      // SELECT PROTOCOL Z-MODEM
      // ENTER OR S TO START
      // RUN THE RECV_STEP 0
    }
    ...
    if (RECV_STEP === 0) {
      ...
      if (this._NextByte === this.ZRQINIT) { // HERE
        try {
          this._Connection.writeString(this.packetZRINIT(0)); // HERE WE SEND THE ZRINIT PACKET
          console.log('-> ZRINIT after received ZRQINIT ' + this.packetZRINIT(0));
          this._Connection.flush();
          recvPacketCount=0; // RESET PACKET COUNT
          RECV_STEP = 1;
        } catch (ioe1) {
          this.HandleIOError(ioe1);
    ...
    if (RECV_STEP === 1) {
      if (this._Connection.bytesAvailable !== 0) {
        ...
        try {
          ...
          var Packet = new ByteArray();
          this._Connection.readBytes(Packet, 0, BlockSize);
          packetHex=this.printBytesAsHex(Packet,0,Packet.length);
          frameBegin=this.findHexSeq(packetHex[0],'18','41'); // ZBIN
          console.log("FBIN " + frameBegin + " CONTENT: " + packetHex[1]);
          if (frameBegin == 2) { // Possible SEXYZ
            Packet.position=0;
          } else {
            Packet.position=0;
    ...
    if (RECV_STEP === 2) { // SEND ZRPOS
      ...
      ZRPOS_PKT = this.packetZRPOS(0);
      this._Connection.writeString(ZRPOS_PKT); // ZRPOS PACKET
      this._Connection.flush();
      //console.log("-> ZRPOS SENT: " + ZRPOS_PKT);

      RECV_STEP=3;
             
    }

    var data; // DATA FROM ZDATA FRAME
    var zDataUE = new ByteArray(); // UnEscaped
    var escCount = 0;
    var dataCalcCRC = new ByteArray(); // ZDATA 1-st Frame for CRC16  
    var dataCalcCRCFrame = new ByteArray(); // ZDATA 2-nd Frame for CRC16  

    if (RECV_STEP === 3) { // RECEIVE ZDATA

      ZDATA_AVAIL = this._Connection.bytesAvailable;
      ...
      if (ZDATA_AVAIL>16) {
        ...
        zData = this.receivePacket(ZDATA_AVAIL);

        zDataHex=this.printBytesAsHex(zData,0,zData.length);
       
        zDataHdrPos=this.findHexSeq(zDataHex[0],'18','41'); // ZBIN
        zDataFrameHexPos=this.findHexSeq(zDataHex[0],'18','42'); // ZHEX
        zDataFrameEnd=this.findHexSeq(zDataHex[0],'18','68'); // ZRCE
        zDataFrameNonStop=this.findHexSeq(zDataHex[0],'18','69'); // ZCRCG
        zDataCRCPos=this.findHexSeq(zDataHex[0],'18','6a'); // ZCRCQ
        if (!zDataCRCPos) { 
          zDataCRCPos=this.findHexSeq(zDataHex[0],'18','6b'); // ZCRCW
          //console.log("Found ZCRCW");
        }

        // The CRC16 is calculated from binary data for binary packets not hexadecimal
        // verify header CRC16 
        if (zDataHdrPos) {
          headerCRCData=this.bytesSubFrame(zData,zDataHdrPos,zDataHdrPos+9); // Get Header
          //crcPos=this.findHexSeq(zDataHex[0],'18','69'); // ZCRCG
          headerCRC=this.bytesSubFrame(headerCRCData,(headerCRCData.length-2),headerCRCData.length); // Extract CRC
          headerCRCDataCalc=this.bytesSubFrame(headerCRCData,2,(headerCRCData.length-2));
          //console.log("HEADER CRC DATA: " + this.printBytesAsHex(headerCRCData,0,headerCRCData.length)[0]);
          //console.log("HEADER CRC EXTRACTED: " + this.printBytesAsHex(headerCRC,0,headerCRC.length)[1]);
          //console.log("HEADER CRC DATA CALC: " + this.printBytesAsHex(headerCRCDataCalc,0,headerCRCDataCalc.length)[0]);
          //console.log("HEADER CRC CALCULATED: " + this.IToHex4(this.calcCRC(headerCRCDataCalc)));
          //console.log("HEADER CRC DATA: " + this.printBytesAsHex(headerCRCData,0,headerCRCData.length)[0]);
          hexHdrCRC=this.printBytesAsHex(headerCRC,0,headerCRC.length)[0][0]+this.printBytesAsHex(headerCRC,0,headerCRC.length)[0][1] 
	  if (hexHdrCRC == this.IToHex4(this.calcCRC(headerCRCDataCalc))) {
            console.log("HEADER CRC O.K." + hexHdrCRC + " = " + this.IToHex4(this.calcCRC(headerCRCDataCalc)));
          } else {
            console.log("HEADER CRC NOT O.K." + hexHdrCRC + " " + this.IToHex4(this.calcCRC(headerCRCDataCalc)));
            hexHdrData=this.printBytesAsHex(headerCRCDataCalc,0,headerCRCDataCalc.length)[0]
            console.log("HEADER CRC DATA " + hexHdrData);
          }
        }
        ...
        // The Second packet looks as this:
        if (! (
          (zDataHex[0][0]==="18") || // NO ZDLE
          (zDataHex[0][1]==="18") ||
          (zDataHex[0][2]==="18") ||
          (zDataHex[0][3]==="18") )) { 
          console.log("No ZDLE");
          ...
          // Process Header
          if (zDataHdrPos>0) {
            for (i=zDataHdrPos+1;i<zDataHdrPos+9;i++) {
              if (zDataHex[0][i]=="18") { // ZDLE IN HEADER
                zDataHdrPos++;
              }
            }
            zDataHdrPos+=9;
          }
          if (zDataFrameNonStop>0&&zDataHdrPos===0) {
            zDataHdrPos=zDataFrameNonStop+4;
          }
        }

        // Find the End
        if (zDataFrameEnd===0&&zDataCRCPos===0) { // no start nor end
          zDataFrameEnd=zData.length;
          //console.log("start nor end zDataHdrPos: " + zDataHdrPos + " zDataFrameEnd: " + zDataFrameEnd);
        } else if (zDataFrameEnd===0&&zDataCRCPos!=0) { // ends with crc
          zDataFrameEnd=zDataCRCPos;
          //console.log("ends with CRC zDataHdrPos: " + zDataHdrPos + " zDataFrameEnd: " + zDataFrameEnd);
        } else if (zDataFrameEnd!=0&&zDataCRCPos===0) { // end but not start
          zDataFrameEnd=zDataFrameEnd;
          //console.log("end not start zDataHdrPos: " + zDataHdrPos + " zDataFrameEnd: " + zDataFrameEnd);
        } else if (zDataFrameEnd!=0&&zDataCRCPos!=0) { // start and end
          zDataFrameEnd=zDataCRCPos;
          //console.log("start and end zDataHdrPos: " + zDataHdrPos + " zDataFrameEnd: "  + zDataFrameEnd);
        }

/*
        var packetCRC = new ByteArray(); // This is currently still unsolved

        if (zDataFrameEnd!=0) {
          packetCRC=this.bytesSubFrame(zData,zDataFrameEnd+2,zDataFrameEnd+4);
          packetCRC.position = 0;
          CRC1 = packetCRC.readUnsignedByte();
          CRC2 = packetCRC.readUnsignedByte();
          console.log(CRC1 + ' ' + CRC2);
          unpackedCRC = CRC1 + CRC2;
          //console.log("Unpacked CRC: " + this.IToHex4(unpackedCRC));
        }
*/

        //console.log("DETECT HPos " + zDataHdrPos + " FraEnd " + zDataFrameEnd + " CRCPos " + zDataCRCPos + " NonSto " + zDataFrameNonStop );

        dataPacket=this.bytesSubFrame(zData,zDataHdrPos,zDataFrameEnd); // Extract data

        dataPacket=this.bytesUnframe(dataPacket,this._TotalBytesReceived); // Remove ZDLE's from data
        dataPacket=this.bytesUnescape(dataPacket); // Unescape
   
        this._TotalBytesReceived += dataPacket.length; // currently Received
        this._File.data.writeBytes(dataPacket,this._TotalBytesReceived,dataPacket.length);
        ...
        missingBytes = senderFileSize - this._File.data.length; // Too few bytes received

        ZCRCGPos=this.findHexSeq(zDataHex[0],'18','69'); // ZCRCG - CRC next, frame continues nonstop
        ZCRCWPos=this.findHexSeq(zDataHex[0],'18','6b'); // ZCRCW - CRC next, ZACK expected, end of frame 

        if (ZCRCWPos) {

          dataCRC=this.bytesSubFrame(zData,ZCRCWPos+2,(ZCRCWPos+4)); // Get Header
          hexDataCRC=this.printBytesAsHex(dataCRC,0,dataCRC.length)[0][0]+this.printBytesAsHex(dataCRC,0,dataCRC.length)[0][1] 
          //console.log("ZCRCW DATA CRC IN HEX OF DATA PACKET: " + hexDataCRC);
          headerCRCData=this.bytesSubFrame(zData,zDataHdrPos,zDataHdrPos+9); // Get Header

          begin=zDataHdrPos;
  
          var escCount=0;
          var begin=zDataHdrPos;
          var end=0;
          if (zDataHex[0][ZCRCWPos+2]=="18" || zDataHex[0][ZCRCWPos+3]=="18") { // ONE CRC IS ESCAPED
            end=ZCRCWPos+3;
            escCount+=3;
          } else {
            end=ZCRCWPos+2;
          ...
          if (zDataHex[0][ZCRCWPos+2]=="18" && zDataHex[0][ZCRCWPos+4]=="18") { // BOTH CRC's HAS ZDLE
            end++;
            escCount++;
          }
          end=ZCRCWPos; // END BEFORE PACKET ENDS ... JUST THE ZCRCG Has to be solved
          if (ZCRCGPos) { // This is the middle of the packet from Mystic
            ZDLEEPos=this.findHexSeq(zDataHex[0],'18','58'); // ZLDEE Location
            end=ZDLEEPos; // End at middle in the packet
            dataCalcCRC.postition=0;
            dataCalcCRC.writeBytes(zData,begin,end-begin-3); // Copy Until 5 bytes prior ZDLEE
            
            // from last 3 bytes remove 0x18 ZDLE bytes
            // END OF FRAME PATCH
            hexDataCalcCRC=this.printBytesAsHex(dataCalcCRC,0,dataCalcCRC.length); // Get Header
            //console.log("dataCalcCRC [" + dataCalcCRC.length + "] " + hexDataCalcCRC[1]);
            dataCalcCRC.positition=dataCalcCRC.length;
            dataCalcCRC.writeByte(0x69);
            //console.log("dataCalcCRC added 0x69 to " + ZCRCGPos + " at byte: " + dataCalcCRC.length);
            hexDataCalcCRC=this.printBytesAsHex(dataCalcCRC,0,dataCalcCRC.length); // Get Header
            //console.log("dataCalcCRC [" + dataCalcCRC.length + "] " + hexDataCalcCRC[1]);
            dataCalcCRC.positition=dataCalcCRC.length;
            dataCalcCRC.writeByte(0x94);
            //console.log("dataCalcCRC added 0x94 to " + ZCRCGPos + " at byte: " + dataCalcCRC.length);
            dataCalcCRC.positition=dataCalcCRC.length;
            dataCalcCRC.writeByte(0x18);
            //console.log("dataCalcCRC added 0x18 to " + ZCRCGPos + " at byte: " + dataCalcCRC.length);
            //dataCalcCRC.positition=dataCalcCRC.length;
            //dataCalcCRC.writeByte(0x58);

            //console.log("dataCalcCRC after END OF FRAME PATCH " + hexDataCalcCRC[1]);
            // First part of the frame is patched... no we want to add the second part of the frame. The one that ends
            // with  18 6b ( ZCRCW 0x6b CRC next, ZACK expected, end of frame )

            // The second part of frame is added to the first one
            dataCalcCRCFrame.position=0;
            dataCalcCRCFrame=this.bytesSubFrame(zData,(ZDLEEPos+2),ZCRCWPos); // Get second part of frame
            //console.log("dataCalcCRCFrame sz: " + dataCalcCRCFrame.length + " from : " + (ZDLEEPos+2) + " to: " + ZCRCWPos);

            dataCalcCRC.position = dataCalcCRC.length;
            dataCalcCRC.writeBytes(dataCalcCRCFrame,0,dataCalcCRCFrame.length);

            //console.log("dataCalcCRC before " + dataCalcCRC.length);
            //dataCalcCRC.positition=dataCalcCRC.length;
            dataCalcCRC.writeByte(0x6b);
            //console.log("dataCalcCRC added 0x6b to " + dataCalcCRC.length);

            hexDataCalcCRC=this.printBytesAsHex(dataCalcCRC,0,dataCalcCRC.length); // Get Header
            //console.log("dataCalcCRC Added second part of the frame: " + hexDataCalcCRC[1]);

          }
          //dataCalcCRC=this.bytesDisescape(dataCalcCRC); // Get Data Packet Content


          //dataCalcCRCFrame.position = dataCalcCRCFrame.length;
          //dataCalcCRCFrame.writeByte(0x6b);
          //console.log("dataCalcCRC added 0x6b to " + dataCalcCRCFrame.length-1 + " size " + dataCalcCRCFrame.length);

          // add the second part of the frame to the first part
          //dataCalcCRC.writeBytes(dataCalcCRCFrame,0,dataCalcCRCFrame.length); // Copy Until 5 bytes prior ZDLEE
        
          //dataCalcCRC=this.bytesUnescape(dataCalcCRC); // Get Data Packet Content
          //hexDataCalcCRC=this.printBytesAsHex(dataCalcCRC,dataCalcCRC,dataCalcCRC.length); // Get Header
          //console.log("ZRCRCQ dataCalcCRC B: " + begin + " E: " + end + " Calculated from: " + hexDataCalcCRC[1]);

          hexCalcDataCRC = this.IToHex4(this.calcCRC(dataCalcCRC,0,dataCalcCRC.length)); // Calculate Data CRC
          //console.log(this.printBytesAsHex(dataCalcCRC,0,dataCalcCRC.length)[1]);
          //console.log("ZCRCW DATA CRC CALCULATED: " + hexCalcDataCRC + " CRC: " + hexCalcDataCRC);

	  if (hexDataCRC == hexCalcDataCRC) {
            console.log("ZCRCW DATA CRC O.K. IN PACKET #" + recvPacketCount + " RETRY #" + retryPacketCount + " PKT #" + retryPacketCount + " DataCRC " + hexDataCRC + " = CalcDataCRC " + hexCalcDataCRC);
            retryPacketCount=0;
          } else {
            console.log("ZCRCW DATA CRC NOT O.K. IN PACKET #" + recvPacketCount + " RETRY #" + retryPacketCount + " PKT #" + retryPacketCount + " DataCRC " + hexDataCRC + " != CalcDataCRC " + hexCalcDataCRC);
            retryPacketCount++;
          }
        ...
        // The CRC16 is calculated from binary data for binary packets not hexadecimal
        ZCRCEPos=this.findHexSeq(zDataHex[0],'18','68'); // ZRCE - CRC next, frame ends, header packet follows
        if (ZCRCEPos) {

          dataCRC=this.bytesSubFrame(zData,ZCRCEPos+2,(ZCRCEPos+4)); // Get Header
          hexDataCRC=this.printBytesAsHex(dataCRC,0,dataCRC.length)[0][0]+this.printBytesAsHex(dataCRC,0,dataCRC.length)[0][1] 
          //console.log("DATA CRC IN HEX OF DATA PACKET: " + hexDataCRC);
          headerCRCData=this.bytesSubFrame(zData,zDataHdrPos,zDataHdrPos+9); // Get Header

          begin=zDataHdrPos;
          end=ZCRCEPos+2;
          dataCalcCRC=this.bytesSubFrame(zData,begin,end); // Get Header
          dataCalcCRC=this.bytesDisescape(dataCalcCRC); // Get Data Packet Content
          hexDataCalcCRC=this.printBytesAsHex(dataCalcCRC,dataCalcCRC,dataCalcCRC.length); // Get Header
          //console.log("dataCalcCRC B: " + begin + " E: " + end + " Calculated from: " + hexDataCalcCRC[1]);

          begin=zDataHdrPos;
          end=ZCRCEPos;
          dataCalcCRC=this.bytesSubFrame(zData,begin,end); // Get Header
          dataCalcCRC.position = end;
          dataCalcCRC.writeByte(0x68);
          dataCalcCRC=this.bytesUnescape(dataCalcCRC); // Get Data Packet Content
          hexDataCalcCRC=this.printBytesAsHex(dataCalcCRC,dataCalcCRC,dataCalcCRC.length); // Get Header
          //console.log("dataCalcCRC B: " + begin + " E: " + end + " Calculated from: " + hexDataCalcCRC[1]);
          ...
          hexCalcDataCRC = this.IToHex4(this.calcCRC(dataCalcCRC,0,dataCalcCRC.length)); // Calculate Data CRC
          //console.log(this.printBytesAsHex(dataCalcCRC,0,dataCalcCRC.length)[1]);
          //console.log("DATA CRC CALCULATED: " + hexCalcDataCRC + " CRC: " + hexCalcDataCRC);

	  if (hexDataCRC == hexCalcDataCRC) {
            console.log("ZCRCE DATA CRC O.K. IN PACKET #" + recvPacketCount + " RETRY #" + " PKT #" + retryPacketCount + " DataCRC " + hexDataCRC + " = CalcDataCRC " + hexCalcDataCRC);
            retryPacketCount=0;
          } else {
            console.log("ZCRCE DATA CRC NOT O.K. IN PACKET #" + recvPacketCount + " RETRY #" + " PKT #" + retryPacketCount + " DataCRC " + hexDataCRC + " != CalcDataCRC " + hexCalcDataCRC);
            retryPacketCount++;
          }
        ...
        //console.log("RECEIVED PACKET [" + zData.length + "] : " + zDataHex[1]);
        //console.log("ACTUAL DATA PACKET [" + dataPacket.length + "] : " + this.printBytesAsHex(dataPacket,0,dataPacket.length)[1]);
        console.log("TOTAL BYTES RECEIVED: " + this._TotalBytesReceived); 
        if (dataPacket.length!=2048) {
          console.log('MISSING BYTES: ' + missingBytes + ' of ' + senderFileSize + ' PKT#: ' + recvPacketCount + " FILEPOS: " + this.IToHex8(this._TotalBytesReceived) + " PKTLEN: " + dataPacket.length);
        }
     
        if (missingBytes >= 0) {
          var ZACK_PKT;
          //var ZRPOS_PKT;
          ZACK_PKT = this.packetZACK(this._File.data.length);
          //ZACK_PKT = this.packetZACK(this._File.data.length);
          //ZRPOS_PKT = this.packetZRPOS(0);
          this._Connection.writeString(ZACK_PKT); // ZACK PACKET
          //this._Connection.writeString(ZRPOS_PKT); // ZRPOS PACKET
          //console.log("-> ZACK SENT: " + ZACK_PKT + " POS: " + this._File.data.length + " H: " + this._File.data.length);
          //console.log("-> ZRPOS SENT: " + ZRPOS_PKT + " POS: " + this._File.data.length + " H: " + this.IToHex8(this._File.data.length));
          zackSent=1;
        }

        /*
        if (zDataFrameHexPos>0) {
          if (zDataHex[0][zDataFrameHexPos+2]) {
              if (zDataHex[0][i]=="18") { // ZDLE IN HEADER
      ...
      if (missingBytes == 0) {
        RECV_STEP = 4; // DATA DOWNLOADED
        console.log("MISSING BYTES " + missingBytes + " FILE DOWNLOADED.");
      } else if (missingBytes < 0) {
        this.Cancel('ZMODEM TRANSFER FAILED TOO MUCH BYTES');
        console.log('ZMODEM TRANSFER FAILED TOO MUCH BYTES');
      } else {
        RECV_STEP = 3; // GET NEXT DATA
        if (zDataHdrPos>0) {
          if (zDataHex[0][3]=="0b") { // ZEOF
            console.log("Sender acknowledges end of file");
            if (missingBytes>0) {
              this.Cancel('ZMODEM TRANSFER FAILED TOO FEW BYTES');
              console.log('ZMODEM TRANSFER FAILED TOO FEW BYTES');
            } else if (missingBytes<0) {
              this.Cancel('ZMODEM TRANSFER FAILED TOO MUCH BYTES');
              console.log('ZMODEM TRANSFER FAILED TOO MUCH BYTES');
            }
            RECV_STEP = 4; // DATA DOWNLOADED // DEBUG
          }
        }
      }
    }

    if (RECV_STEP === 4) { // RECEIVE ZEOF 

      // 10 - INSUFF

      console.log("RECEIVE ZEOF DONE...");

      // var Packet = new ByteArray();

      // try {
      //   c = this.hexToChar(Packet.readUnsignedByte()); // ZEOF 
      //   console.log(c);
      // } catch (ZEOF_EXC) {
      //   console.log("ZEOF_EXC NO WAY TO READ UNSIGNED BYTE");
      // }


      // BlockSize = this._Connection.bytesAvailable;
      // console.log("ZEOF_EXC BYTES AVAIL: " + BlockSize);

      // this._Connection.readBytes(Packet, 0, BlockSize);
      // console.log("<-- ZEOF RECEIVED: " + Packet + " PACKET SIZE: " + BlockSize);

      RECV_STEP = 5;
            
    }

    if (RECV_STEP === 5) { // SEND ZRINIT (#2) 

      this._Connection.writeString(this.packetZRINIT(0)); // HERE WE SEND THE ZRINIT PACKET
      console.log('-> ZRINIT after received ZEOF ' + this.packetZRINIT(0));
    ...
    if (RECV_STEP === 6) { // RECEIVE ZFIN

      ZFIN_AVAIL = this._Connection.bytesAvailable;
      console.log("ZFIN: " + ZFIN_AVAIL);
      if (ZFIN_AVAIL>0) {
        zFin = this.receivePacket(ZFIN_AVAIL);
        console.log("ZFIN PACKET: " + zFin);
      }
      RECV_STEP = 7;
    }

    if (RECV_STEP === 7) { // SEND ZFIN

      ZFIN_PKT = this.packetZFIN();
      this._Connection.writeString(ZFIN_PKT); // ZFIN PACKET
      this._Connection.flush();
      console.log("-> ZFIN SENT: " + ZFIN_PKT);

      this._TotalBytesReceived = this._File.data.length;

      this.lblFileReceived.Text = 'File Recv: ' + StringUtils.AddCommas(this._File.data.length) + ' bytes';
      this.pbFileReceived.Value = this._File.data.length;

      this.CleanUp('ZMODEM TRANSFER COMPLETED. RECEIVED ' + this._File.data.length + ' bytes of ' + senderFileSize);

      this._Files.push(this._File);

      if (this._File.data.length == senderFileSize) { // Too few bytes received
        this.SaveFile(0); // FOR NOW JUST ONE FILE
      } else {
        console.log("Won't save file. The file Size Differs");
      }

      //this.SaveFile(this._Files.length - 1); // LATER USER
                       
      console.log('ZMODEM TRANSFER COMPLETED. RECEIVED ' + this._File.data.length + ' bytes of ' + senderFileSize);
...
var YModemReceive = (function () {
    ...
    YModemReceive.prototype.Download = function () {
        var _this = this;
        this._Timer = setInterval(function () { _this.OnTimer(); }, 75); // HERE TIMER
        //this._Timer = setInterval(function () { _this.OnTimer(); }, 20);
        this._Crt.HideCursor();
...
var fTelnetOptions = (function () {
    function fTelnetOptions() {
        ...
        this.Port = 1123;
        this.ProxyHostname = 'proxy-nl.ftelnet.ca';
        this.ProxyPort = 1123;
...
var fTelnetClient = (function () {
    function fTelnetClient(containerId, options) {
        ...
        this._MenuButton.addEventListener('mouseover', function(c) { _this.OnMenuMouseOver(); return false; }, false );
        this._MenuButton.addEventListener('mouseout', function(c) { _this.OnMenuMouseOut(); return false; }, false );
        ...
        this._ConnectButton.innerHTML = 'Go!';
        this._ConnectButton.addEventListener('click', function (e) { _this.Connect(); e.preventDefault(); return false; }, false);
        this._ConnectButton.addEventListener('mouseover', function(c) { _this.OnConnectMouseOver(); return false; }, false );
        this._ConnectButton.addEventListener('mouseout', function(c) { _this.OnConnectMouseOut(); return false; }, false );
        ...
        if ((typeof YModemReceive !== 'undefined') && (typeof YModemSend !== 'undefined')) {
            ...
            MenuButtonsDownload.innerHTML = 'YM Down';
            MenuButtonsDownload.addEventListener('click', function (me) { _this.YDownload(); me.preventDefault(); return false; });
            MenuButtonsRow3Cell1.appendChild(MenuButtonsDownload);
            ...
            MenuButtonsUpload.innerHTML = 'YM Upload';
            MenuButtonsUpload.addEventListener('click', function (me) { _this.Upload(); me.preventDefault(); return false; });
            MenuButtonsRow3Cell2.appendChild(MenuButtonsUpload);
            MenuButtonsRow3.appendChild(MenuButtonsRow3Cell2);

            MenuButtonsTable.appendChild(MenuButtonsRow3);
        }
        if ((typeof ZModemReceive !== 'undefined')) {
            var MenuButtonsRow3 = document.createElement('tr');
            var MenuButtonsRow3Cell2 = document.createElement('td');
            var MenuButtonsDownload = document.createElement('a');
            MenuButtonsDownload.href = '#';
            MenuButtonsDownload.innerHTML = 'ZM Down';
            MenuButtonsDownload.addEventListener('click', function (me) { _this.ZDownload(); me.preventDefault(); return false; });
    ...
    fTelnetClient.prototype.Connect = function () {
        ...
        if (this._Options.ProxyHostname === '') {
            ...
            this._StatusBar.style.backgroundColor = 'black';
    ...
    fTelnetClient.prototype.YDownload = function () {
        var _this = this;
        if (typeof this._MenuButtons !== 'undefined') {
            this._MenuButtons.style.display = 'none';
        }
    ...
    fTelnetClient.prototype.ZDownload = function () {
        var _this = this;
        if (typeof this._MenuButtons !== 'undefined') {
            this._MenuButtons.style.display = 'none';
        }
        ...
        this._ZModemReceive = new ZModemReceive(this._Crt, this._Connection);
        if (typeof this._Timer !== 'undefined') {
            clearInterval(this._Timer);
            delete this._Timer;
        }
        this._ZModemReceive.ontransfercomplete.on(function () { _this.OnDownloadComplete(); });
        this._ZModemReceive.Download();
    };
    ...
    fTelnetClient.prototype.OnMenuMouseOver = function () {
      this._ConnectButton.style.color = "#989898";
      this._MenuButton.style.color = "#dfdfdf";
      this._StatusBarLabel.style.color = "#989898";
    };
    fTelnetClient.prototype.OnMenuMouseOut = function () {
      this._ConnectButton.style.color = "#101010";
      this._MenuButton.style.color = "#101010";
      this._StatusBarLabel.style.color = "#101010";
    };
     fTelnetClient.prototype.OnConnectMouseOver = function () {
      this._ConnectButton.style.color = "#dfdfdf";
      this._MenuButton.style.color = "#989898";
      this._StatusBarLabel.style.color = "#989898";
    };
    fTelnetClient.prototype.OnConnectMouseOut = function () {
      this._ConnectButton.style.color = "#101010";
      this._MenuButton.style.color = "#101010";
      this._StatusBarLabel.style.color = "#101010";
    };
     fTelnetClient.prototype.OnConnectionClose = function () {
        ...
        //this._StatusBar.style.backgroundColor = 'darkGrey';
        this._StatusBar.style.backgroundColor = 'black';
    ...
    fTelnetClient.prototype.OnConnectionSecurityError = function () {
        ...
        if (this._Options.ProxyHostname === '') {
            ...
            //this._StatusBar.style.backgroundColor = 'darkGray';
            this._StatusBar.style.backgroundColor = 'black';
            this._ClientContainer.style.opacity = '0.5';
        }
        else {
    ...
    fTelnetClient.prototype.OnMenuButtonClick = function () {
        ...
        this._MenuButtons.style.left = Offset.getOffset(this._MenuButton).x + this._MenuButton.clientWidth - 23 + 'px'; // HERE
        this._MenuButtons.style.top = Offset.getOffset(this._MenuButton).y - this._MenuButtons.clientHeight - 4 + 'px'; // HERE
    };
