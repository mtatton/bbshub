...
ZRINITHeader = function() {
  ZPAD='*';
  ZDLE=String.fromCharCode(24);
  ZHEX='B';
  CR=0x0D;
  LF=0x0A;
  PKT_LEN=1024; // DATA SUBPACKET LENGTH
  ZRINIT=0x1; // PACKET TYPE
  TRCAPA=0; // TRANSFER CAPABILITIES
  PKT_TYPE=ZRINIT;

  ZF1=IToHex(TRCAPA >> 0x8); // A2
  ZF0=IToHex(TRCAPA - ZF1 * 0xFF); // A3
  ZP1=IToHex(PKT_LEN >> 0x8); // A1
  ZP0=IToHex(PKT_LEN - ZP1 * 0xFF); // A0

  HEX_HDR=IToChar(ZPAD)+IToChar(ZPAD)+IToChar(ZDLE)+IToChar(ZHEX);
  ZRINIT_HDR=HEX_HDR+PKT_TYPE+ZF1+ZF0+ZP1+ZP0;
  PKT_END = IToHex(CR) + IToHex(LF);
  ZRINIT_HDR=ZRINIT_HDR+IToHex4(CRC.Calculate16(hexToBytes(ZRINIT_HDR)))+PKT_END;
  console.log('ZRINIT_HDR' + ZRINIT_HDR);
  return (ZRINIT_HDR);
}
